...
export function registerCatalogoRoutes(app: Express) {
  ...
  app.get("/api/inventario/public", async (req: Request, res: Response) => {
    try {
      const vehicles = await storage.query(
        `SELECT 
          id, marca, modelo, variante, anio, color,
      ...
      const publicVehicles = vehicles.rows.map((v: any) => ({
        id: v.id,
        marca: v.marca,
        modelo: v.modelo,
        variante: v.variante,
