 ...
 * 
 * Ejecutar local:  npx tsx test/name-matcher.ts
 * Ejecutar prod:   POST /api/test/name-matcher
 */
...
export function runNameMatcherTests() {
  ...
  function testMatch(id: string, description: string, a: string, b: string, expected: boolean) {
    ...
    if (!pass) {
      const rawA = normalizeName(a).split(" ").filter((t: string) => t.length > 0).map(expandAbbreviation);
      const rawB = normalizeName(b).split(" ").filter((t: string) => t.length > 0).map(expandAbbreviation);
